...
class MedCLIPModel(nn.Module):
    def __init__(self,
        ...
        # 🟢 NEW: decide device once (CPU in your case)
    ...
    def encode_text(self, input_ids=None, attention_mask=None):
        ...
        #Start of CPU and GPU agnostic encode_text function
        # If no text provided, return None (for image-only usage)
        ...
        #End of CPU and GPU agnostic encode_text function
    ...
    #Start of CPU and GPU agnostic encode_image function
    def encode_image(self, pixel_values=None):
        """
        Encode images using the vision backbone and L2-normalize the embeddings.
        Device-agnostic: moves pixel_values to the same device as the model.
        """
        if pixel_values is None:
            return None

        # Use the model's device (CPU on your laptop, CUDA on Kaggle, etc.)
        device = next(self.parameters()).device
        ...
        return img_embeds
    #End of CPU and GPU agnostic encode_image function
    ...
    '''def forward(self,
        ...
        **kwargs,
        ...
        # For your use case (Stage 0), we expect both to be non-None
        if img_embeds is None or text_embeds is None:
            raise ValueError("Both image (pixel_values) and text (input_ids) must be provided for this forward() call.")
    ...
    #Start of CPU and GPU agnostic forward function
    def forward(self,
        input_ids=None,
        pixel_values=None,
        attention_mask=None,
        ...
        **kwargs,
        ...
        """
        Joint image–text forward for CLIP-style usage.
        Device-agnostic: tensors are moved to the same device as the model.
        """
        # Decide device from model weights (more robust than self.device)
        device = next(self.parameters()).device
        ...
        img_embeds = self.encode_image(pixel_values) if pixel_values is not None else None
        text_embeds = self.encode_text(input_ids, attention_mask) if input_ids is not None else None

        # For CLIP contrastive loss we expect both; otherwise we just return what we can
        if img_embeds is None or text_embeds is None:
            return {
    ...
    #End of CPU and GPU agnostic forward function
...
class PromptClassifier(nn.Module):
    ...
    def forward(self, pixel_values=None, prompt_inputs=None, **kwargs):
        ...
        Device-agnostic: tensors moved to the same device as the MedCLIP model.
        """
        if pixel_values is None or prompt_inputs is None:
            raise ValueError("Both pixel_values and prompt_inputs must be provided.")
        ...
        for cls_name, cls_text in prompt_inputs.items():
            ...
            if self.ensemble:
                cls_sim = torch.mean(logits, dim=1)
            else:
                cls_sim = torch.max(logits, dim=1)[0]
...
class PromptTuningClassifier(nn.Module):
    ...
    def forward(self, pixel_values=None, prompt_inputs=None, labels=None, return_loss=True, **kwargs):
        """
        Prompt-tuning classifier on top of MedCLIP.
        Device-agnostic: everything moved to the MedCLIP model device.
        """
        if pixel_values is None or prompt_inputs is None:
            raise ValueError("Both pixel_values and prompt_inputs must be provided.")
