...
- Enumerates IU-CXR images via projections CSV (expects columns: image/view/filename/projection etc.).
...
# --- Repo import hardening + quiet logs (MUST be first, before other imports) ---
...
# Quieter tokenizers log with DataLoader workers
os.environ.setdefault("TOKENIZERS_PARALLELISM", "false")

# Avoid CPU oversubscription on Kaggle
try:
    import torch
...
def build_image_list(cfg: Dict) -> Tuple[List[str], List[int]]:
    """
    Reads projections_csv; tolerates header variants:
    - image/img/filename, view/projection/view position
    Maps labels → 0: frontal (AP/PA), 1: lateral.
    Returns absolute image paths and int labels.
    """
    ds = cfg["dataset"]
    ...
    with open(csv_path, "r", newline="") as f:
        ...
        view_col = cols.get("view") or cols.get("projection") or cols.get("view position")
        if not img_col or not view_col:
            raise ValueError("projections_csv must have columns like {image|filename} and {view|projection}.")
...
def make_collate_fn(processor: CLIPProcessor):
    """
    Dataset gives: (PIL.Image (RGB), label:int, path:str)
    Collate returns:
      pixel_values [B,3,224,224], labels [B], paths [B]
    """
    def _collate(batch):
        imgs, labels, paths = zip(*batch)
        proc = processor(images=list(imgs), return_tensors="pt")
        pixel_values = proc["pixel_values"]  # CLIP mean/std applied
...
if __name__ == "__main__":
    ...
    # Encode text prompts once (zero-shot)
    prompt_texts = [
    ...
    with torch.no_grad():
        ...
        text_features = text_features / text_features.norm(dim=-1, keepdim=True)  # normalize once
    ...
    # Use AMP only on GPU (safer across torch versions)
